    ...
    <?php if (isset($user)): ?>
      ...
      <div class="body">
        <div class="header">
          ...
          <h3 style="margin: 0; padding: 0; color: #cbc8c7;"><u>Edit Item</u></h3>
          <br>
        </div>

        <h1 style="margin:0; padding:0; color: #cbc8c7;">Provide Modified Item Information</h1>

        <form action="process_edititem.php" method="post" id="item">
          <div>
            <input type='hidden' name='id' value='<?php echo htmlspecialchars($item["ID"]);?>'/>
          </div>
          <div>
            <label for="name">Name</label>
            <input type="text" id="name" name="name"  placeholder="Enter the new name of the item"
          ...
          <div>
            ...
            <input type="number" id="mina" name="mina" placeholder="Enter the minimum amount of the item you like in your inventory"
                   value="<?php echo $item["Min_amount"] ?>" style="margin-right: 0px !important; width: 100%">
          </div>
          <div>
            ...
            <input type="number" id="maxa" name="maxa" placeholder="Enter the maximum amount of the item you like in your inventory"
                   value="<?php echo $item["Max_amount"] ?>" style="margin-right: 0px !important; width: 100%">
          </div>
        ...
        <?php if ($shop["role_name"] === "ADMIN" || $shop["role_name"] === "MANAGER"): ?>
        <form action="process_deleteitem.php" method="post">
          <div>
            <input type='hidden' name='id' value='<?php echo htmlspecialchars($item["ID"]);?>'/>
          </div>
          <div class="header">
            <button style="margin:10; padding:10; background-color: rgba(0, 0, 0, 0.0); border: none;">
              <br><h3 style="margin:0; padding:0; color: red;">Delete Item</h3>
            </button>
          </div>
        </form>
        <?php endif; ?>
